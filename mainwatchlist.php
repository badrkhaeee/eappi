...
// --------------------------------------------------------
	...
	echo '	<div id="watchlist-headertextbutton"><input id="watchlist-config-button" type="button" value="&#160;" onclick="LoadObject(\'watchlistconfig\',\'\',\'\',\'\',\'' . _glt('Watchlist configuration') . '\',\'images/element16/watchlistconfig.png\')" title="' . _glt('Configure the Watchlist') . '"/></div>';
	echo '	</div>';
	...
	$iFeatureVerified	= SafeGetArrayItem1DimInt($aWatchList, 'featureverified');
	$iFeatureRequested	= SafeGetArrayItem1DimInt($aWatchList, 'featurerequested');
	$iFeatureCompleted 	= SafeGetArrayItem1DimInt($aWatchList, 'featurecompleted');
	$iFeatureNew	 	= SafeGetArrayItem1DimInt($aWatchList, 'featurenew');
	$iFeatureIncomplete 	= SafeGetArrayItem1DimInt($aWatchList, 'featureincomplete');
	...
	$iDocumentVerified	= SafeGetArrayItem1DimInt($aWatchList, 'documentverified');
	$iDocumentRequested	= SafeGetArrayItem1DimInt($aWatchList, 'documentrequested');
	$iDocumentCompleted 	= SafeGetArrayItem1DimInt($aWatchList, 'documentcompleted');
	$iDocumentNew	 	= SafeGetArrayItem1DimInt($aWatchList, 'documentnew');
	$iDocumentIncomplete 	= SafeGetArrayItem1DimInt($aWatchList, 'documentincomplete');
	...
	$sFeatureVerifiedCrit= SafeGetArrayItem1Dim($aWatchList, 'featureverifiedcrit');
	$sFeatureRequestedCrit= SafeGetArrayItem1Dim($aWatchList, 'featurerequestedcrit');
	$sFeatureCompletedCrit= SafeGetArrayItem1Dim($aWatchList, 'featurecompletedcrit');
	$sFeatureNewCrit	 	= SafeGetArrayItem1Dim($aWatchList, 'featurenewcrit');
	$sFeatureIncompleteCrit = SafeGetArrayItem1Dim($aWatchList, 'featureincompletecrit');
	...
	$sDocumentVerifiedCrit= SafeGetArrayItem1Dim($aWatchList, 'documentverifiedcrit');
	$sDocumentRequestedCrit= SafeGetArrayItem1Dim($aWatchList, 'documentrequestedcrit');
	$sDocumentCompletedCrit= SafeGetArrayItem1Dim($aWatchList, 'documentcompletedcrit');
	$sDocumentNewCrit	 	= SafeGetArrayItem1Dim($aWatchList, 'documentnewcrit');
	$sDocumentIncompleteCrit = SafeGetArrayItem1Dim($aWatchList, 'documentincompletecrit');
	...
	if ( $iRecentDiscuss 	> 0 || $iRecentDiagrams > 0 || $iRecentElements 	> 0 || $iRecentReviews 	> 0 ||
	 ...
	 $iFeatureVerified 	> 0 || $iFeatureRequested > 0 || $iFeatureCompleted 	> 0 || $iFeatureNew	> 0 || $iFeatureIncomplete 	> 0 ||
	 $iChangeVerified 	> 0 || $iChangeRequested > 0 || $iChangeCompleted 	> 0 || $iChangeNew	> 0 || $iChangeIncomplete 	> 0 ||
	 $iDocumentVerified > 0 || $iDocumentRequested > 0 || $iDocumentCompleted 	> 0 || $iDocumentNew	> 0 || $iDocumentIncomplete 	> 0 ||
	...
	if ( $iFeatureVerified	> 0 ||
	 $iFeatureRequested > 0 ||
	 $iFeatureCompleted > 0 ||
	 $iFeatureNew 	> 0 ||
	 $iFeatureIncomplete > 0 )
	{
	echo '<tr class="watchlist-line">';
	echo '<td class="watchlist-option-hdr">' . _glt('Features') . '&nbsp;&nbsp;&nbsp;<img alt="" src="images/spriteplaceholder.png" class="propsprite-feature"/></td>';
	echo '</tr>';
	WriteWatchListItem($iFeatureVerified, _glt('%NUM% verified feature%S%'), $sFeatureVerifiedCrit);
	WriteWatchListItem($iFeatureRequested, _glt('%NUM% recently requested feature%S%'), $sFeatureRequestedCrit);
	WriteWatchListItem($iFeatureCompleted, _glt('%NUM% recently completed feature%S%'), $sFeatureCompletedCrit);
	WriteWatchListItem($iFeatureNew, _glt('%NUM% new feature%S%'), $sFeatureNewCrit);
	WriteWatchListItem($iFeatureIncomplete, _glt('%NUM% incomplete feature%S%'), $sFeatureIncompleteCrit);
	}
	...
	if ( $iDocumentVerified	> 0 ||
	 $iDocumentRequested > 0 ||
	 $iDocumentCompleted > 0 ||
	 $iDocumentNew 	> 0 ||
	 $iDocumentIncomplete > 0 )
	{
	echo '<tr class="watchlist-line">';
	echo '<td class="watchlist-option-hdr">' . _glt('Documents') . '&nbsp;&nbsp;&nbsp;<img alt="" src="images/spriteplaceholder.png" class="propsprite-document"/></td>';
	echo '</tr>';
	WriteWatchListItem($iDocumentVerified, _glt('%NUM% verified document%S%'), $sDocumentVerifiedCrit);
	WriteWatchListItem($iDocumentRequested, _glt('%NUM% recently requested document%S%'), $sDocumentRequestedCrit);
	WriteWatchListItem($iDocumentCompleted, _glt('%NUM% recently completed document%S%'), $sDocumentCompletedCrit);
	WriteWatchListItem($iDocumentNew, _glt('%NUM% new document%S%'), $sDocumentNewCrit);
	WriteWatchListItem($iDocumentIncomplete, _glt('%NUM% incomplete document%S%'), $sDocumentIncompleteCrit);
	}
	...
	$sCount = '<a class="w3-link" onclick="OnClickWatchlistItem(\'' . _j($sClause) . '\',\'' . _glt('Watchlist results') . '\')">' . _h($sCount) . '</a>';
	}
