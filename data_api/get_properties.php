...
// --------------------------------------------------------
	...
	{
	$sCommentID	= SafeGetInternalArrayParameter($_POST, 'commentid');
	if (strIsEmpty($sCommentID))
	$sCommentID	= '1';
	...
	if(!isset($bIncludeArtifacts) || (!$bIncludeArtifacts===true))
	AddURLParameter($sParas, 'options', 'exclude_ld_content ');
	if ($sCommentID !== 'all')
	AddURLParameter($sParas, 'comment', $sCommentID);
	if ($sCommentID == 'all')
	$sResourceFeaturesFilter = 'cm';
	if ( !strIsEmpty($sResourceFeaturesFilter) )
	{
	$sResourceFeaturesFilter = str_replace('op', 'op,pr', $sResourceFeaturesFilter);
	...
	GetXMLNodeValue($xnObjProp, 'dcterms:subject', 	$aCommonProps['subject']);
	...
	GetXMLNodeValue($xnObjProp, 'ss:difficulty', 	$aCommonProps['difficulty']);
	GetXMLNodeValue($xnObjProp, 'ss:priority', 	$aCommonProps['priority']);
	GetXMLNodeValue($xnObjProp, 'ss:language', 	$aCommonProps['language']);
	if ($sSbpiid !== null)
	{
	...
	elseif ($xnFeature->nodeName === "ss:files")
	{
	$aFiles = getFiles($xnFeature, $aExternalData);
	}
	elseif ($xnFeature->nodeName === "ss:discussions")
	{
	$aDiscussions = getDiscussions($xnFeature);
	}
	elseif ($xnFeature->nodeName === "ss:comments")
	{
	$aComments = getComments($xnFeature);
	}
	...
	{
	$aDocument = getLinkedDocument($xnFeature);
	}
	elseif ($xnFeature->nodeName === "ss:mfeatures")
	{
	$aFeatures = getFeatures($xnFeature);
	}
	elseif ($xnFeature->nodeName === "ss:changes")
	{
	$aChanges = getChanges($xnFeature);
	}
	elseif ($xnFeature->nodeName === "ss:documents")
	{
	$aDocuments = getDocuments($xnFeature);
	}
	...
	function getFiles($xnFiles, &$aExternalData)
	{
	$aFiles	= array();
	$aExternalData	= array();
	if ($xnFiles->nodeName !== 'ss:files')
	{
	return $aFiles;
	}
	$xnDesc = $xnFiles->firstChild;
	if ($xnDesc != null)
	{
	foreach ($xnDesc->childNodes as $xnMembers)
	{
	$xnFile = $xnMembers->firstChild;
	$sType	= '';
	$sModified 	= '';
	$sDescription	= '';
	$sFilePath	= '';
	...
	$sIdentifier	= '';
	$sResourceIdentifier	= '';
	foreach ($xnFile->childNodes as $xn)
	{
	GetXMLNodeValue($xn, 'dcterms:type', 	$sType);
	GetXMLNodeValue($xn, 'dcterms:modified', 	$sModified);
	GetXMLNodeValue($xn, 'dcterms:description', 	$sDescription);
	GetXMLNodeValue($xn, 'ss:filepath', 	$sFilePath);
	GetXMLNodeValue($xn, 'ss:size', 	$sSize);
	GetXMLNodeValue($xn, 'dcterms:identifier', 	$sIdentifier);
	GetXMLNodeValue($xn, 'ss:resourceidentifier', $sResourceIdentifier);
	}
	$aRow	= array();
	$aRow['type'] 	= $sType;
	$aRow['modified'] 	= $sModified;
	$aRow['description'] 	= $sDescription;
	$aRow['filepath']	= $sFilePath;
	$aRow['size']	= $sSize;
	$aRow['identifier']	= $sIdentifier;
	$aRow['resourceidentifier'] = $sResourceIdentifier;
	$aFiles[] = $aRow;
	}
	}
	return $aFiles;
	...
	GetXMLNodeBoolValue($xn, 'ss:isordered',$sIsOrdered);
	GetXMLNodeBoolValue($xn, 'ss:iscollection',$sIsCollection);
	GetXMLNodeValue($xn, 'ss:length',$sLength);
	GetXMLNodeValue($xn, 'ss:precision',$sPrecision);
	GetXMLNodeValue($xn, 'ss:scale',$sScale);
	GetXMLNodeValue($xn, 'ss:position',$sPosition);
	...
	$aRow['length']  	= $sLength;
	$aRow['precision']  = $sPrecision;
	$aRow['scale']  	= $sScale;
	$aRow['position']  = $sPosition;
	...
	$sParaType  = '<a class="w3-link" onclick="LoadObject(\'' . _j($sClassGUID) . '\',\'false\',\'\',\'\',\'' . _j($sClassName) . '\',\'' . _j($sClassImageURL) . '\')">';
	$sParaType .= '<img src="images/spriteplaceholder.png" class="' . GetObjectImageSpriteName($sClassImageURL) . '" alt="" style="float: none;">&nbsp;' . _h($sClassName) . '</a>';
	}
	else
	{
	$sParaType = _h($sParaType);
	}
	$sCurrentPara = _h($sParaName) . ":" . $sParaType;
	if ( !strIsEmpty($sParaDefault) )
	$sCurrentPara .= "=" . _h($sParaDefault);
	$sCurrentPara .= ", ";
	$sParameters .= $sCurrentPara;
	}
	elseif ( !strIsEmpty($sParaName) &&  strIsEmpty($sParaType) )
	{
	$sParameters .= _h($sParaName) . ", ";
	}
	$aRow	= array();
	$aRow['name']	= $sParaName;
	$aRow['guid']	= $sParaGUID;
	...
	$aRow['link']	= $sLink;
	foreach ($xnScenStep->childNodes as $xnScenExt)
	{
	$sExtLevel	= '';
	$sExtGUID	= '';
	$sExtJoin	= '';
	GetXMLNodeValueAttr($xnScenExt, 'extension', 'level', $sExtLevel);
	GetXMLNodeValueAttr($xnScenExt, 'extension', 'guid', $sExtGUID);
	GetXMLNodeValueAttr($xnScenExt, 'extension', 'join', $sExtJoin);
	if ( !strIsEmpty($sExtLevel) || !strIsEmpty($sExtGUID) || !strIsEmpty($sExtJoin) )
	{
	$aExt = array();
	$aExt['level'] = $sExtLevel;
	$aExt['guid'] = $sExtGUID;
	$aExt['join'] = $sExtJoin;
	$aRow['extensions'][]= $aExt;
	}
	}
	$aSenarioSteps[]= $aRow;
	}
	...
	if ((!empty($aScenarios)) && (array_key_exists('steps',$aScenarios[0])))
	{
	foreach ($aScenarios[0]['steps'] as &$aStep)
	{
	if (array_key_exists('extensions',$aStep))
	{
	foreach ($aStep['extensions'] as &$aExt)
	{
	foreach ($aScenarios as $aScenario)
	{
	$sOSLCGUID = SafeGetArrayItem1Dim($aScenario, 'oslcguid');
	if (($sOSLCGUID) === ('sc_'.$aExt['guid']))
	{
	$aExt['name'] = $aScenario['name'];
	$aExt['type'] = $aScenario['type'];
	}
	}
	foreach ($aScenarios[0]['steps'] as $aBasicStep)
	{
	$sJoin = SafeGetArrayItem1Dim($aExt, 'join');
	if ($sJoin === $aBasicStep['guid'])
	{
	$aExt['joinstep'] = $aBasicStep['level'];
	}
	else if ($sJoin === 'End')
	{
	$aExt['joinstep'] = 'End';
	...
	function getFeatures($xnFeatures)
	{
	$aFeatures = array();
	if ($xnFeatures->nodeName !== "ss:mfeatures")
	{
	return $aFeatures;
	}
	$xnDesc = $xnFeatures->firstChild;
	if ($xnDesc != null)
	{
	$iItemNo = 1;
	foreach ($xnDesc->childNodes as $xnMembers)
	{
	$xnFeature = $xnMembers->firstChild;
	$sName	= '';
	$sNotes	= '';
	$sHistory	= '';
	$sStatus	= '';
	...
	$sParentGUID= '';
	$sGUID	= '';
	foreach ($xnFeature->childNodes as $xn)
	{
	GetXMLNodeValue($xn, 'dcterms:title', 	$sName);
	GetXMLNodeValue($xn, 'dcterms:description', $sNotes);
	GetXMLNodeValue($xn, 'ss:history', 	$sHistory);
	...
	$aRow['oslcguid']	= $sGUID;
	$aRow['guid']	= BuildWebUID('mf', $sParentGUID, $iItemNo);
	...
	function getComments($xnComments)
	{
	$aComments = array();
	if ($xnComments->nodeName !== "ss:comments")
	{
	return $aComments;
	}
	$xnDesc = $xnComments->firstChild;
	if ($xnDesc != null)
	{
	foreach ($xnDesc->childNodes as $xnMembers)
	{
	$xnComment = $xnMembers->firstChild;
	$sCreated	= '';
	$sDescription	= '';
	foreach ($xnComment->childNodes as $xn)
	{
	GetXMLNodeValue($xn, 'dcterms:description', $sDescription);
	...
	function getDocuments($xnDocuments)
	{
	$aDocuments = array();
	if ($xnDocuments->nodeName !== "ss:documents")
	{
	return $aDocuments;
	}
	$xnDesc = $xnDocuments->firstChild;
	if ($xnDesc != null)
	{
	$iItemNo = 1;
	foreach ($xnDesc->childNodes as $xnMembers)
	{
	$xnDocument = $xnMembers->firstChild;
	$sName	= '';
	$sNotes	= '';
	$sHistory	= '';
	$sStatus	= '';
	...
	$sParentGUID= '';
	$sGUID	= '';
	foreach ($xnDocument->childNodes as $xn)
	{
	GetXMLNodeValue($xn, 'dcterms:title', 	$sName);
	GetXMLNodeValue($xn, 'dcterms:description', $sNotes);
	GetXMLNodeValue($xn, 'ss:history', 	$sHistory);
	...
	$aRow['version']	= $sVersion;
	$aRow['oslcguid']	= $sGUID;
	$aRow['guid']	= BuildWebUID('dm', $sParentGUID, $iItemNo);
	$aRow['parentguid']	= $sParentGUID;
	$aDocuments[] 	= $aRow;
	}
	$iItemNo += 1;
