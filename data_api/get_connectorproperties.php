...
// --------------------------------------------------------
	...
	if ($xnObjProp->nodeName === "ss:informationitems")
	{
	$xnDescription = GetXMLFirstChild($xnObjProp);
	if ($xnDescription != null)
	{
	$aInfoItemStereotypes = array();
	foreach ($xnDescription->childNodes as $xnMem)
	{
	$xnInformationItem = GetXMLFirstChild($xnMem);
	foreach ($xnInformationItem->childNodes as $xnItem)
	{
	GetXMLNodeValue($xnItem, 'dcterms:title', $sTitle);
	GetXMLNodeValue($xnItem, 'dcterms:identifier', $sIdentifier);
	GetXMLNodeValue($xnItem, 'dcterms:type', $sType);
	GetXMLNodeValue($xnItem, 'ss:resourcetype', $sResourceType);
	GetXMLNodeValue($xnItem, 'ss:ntype', $sNType);
	AddStereotypeToArray($xnItem, $aInfoItemStereotypes);
	}
	$aInformationItems[] = [
	'title' => $sTitle,
	...
	'imageurl' => GetObjectImagePath($sType, $sResourceType, getPrimaryStereotype($aInfoItemStereotypes), $sNType, '16')
	];
	}
	}
	}
