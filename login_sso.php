...
// --------------------------------------------------------
	...
	SafeStartSession();
	if (strIsEmpty($sOpenIDState)
	|| ($sOpenIDState !== $_SESSION['openIDState']))
	{
	$sErrorMsg = 'Invalid "state" parameter';
	$gLog->Write2Log('login_sso:  THERE IS A PROBLEM  ' . $sErrorMsg);
	$_SESSION['login_error'] = $sErrorMsg;
	RedirectToLogin();
	}
	$sModelNo = SafeGetArrayItem1DimInt($_SESSION, 'model_no');
	if (strIsEmpty($sModelNo))
	{
	$sErrorMsg = 'No model selected';
	$gLog->Write2Log('login_sso:  THERE IS A PROBLEM  ' . $sErrorMsg);
	$_SESSION['login_error'] = $sErrorMsg;
	RedirectToLogin();
	}
